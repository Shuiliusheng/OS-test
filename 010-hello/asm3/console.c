...
int hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	uint cs_base = *((int*)ADR_CODE);
	sTASK *task = task_now();
	if (edx == 1) {
		cons_putchar(cons, eax & 0xff, 1);
	} 
	else if(edx ==2){
		show_string_sheet(cons, (char*)(ebx+cs_base), eax);
	}
	else if(edx ==4){
...
int cmd_app(sCONSOLEINFO *cons, char *cmdline, uint *fat)
{
	...
	if(info!=0){
		...
		if(gdt_code!= 0 && gdt_data!=0){
			char *code = (char *)mm_alloc(memManager, info->size);
			char *data = (char *)mm_alloc(memManager, 64*1024);
			//cs
			*((int *)ADR_CODE)=(int)code;
			file_readdata(fat,code,info->size,info->clustno);
			//通过+0x60让cpu能够区分出当前是处于什么段，用户还是os
			//但是如此区分后，将无法使用farcall/jmp从os跳转到用户
			//为此需要将start_app中的call改成retf，从而跳转到用户
			set_segmdesc(gdt + gdt_code/8, info->size-1, (int)code, AR_CODE32_ER+0x60);
			set_segmdesc(gdt + gdt_data/8, 64*1024-1, (int)data, AR_DATA32_RW+0x60);
			
			//C语言编出来的第五个字节为Hari，然后开始6个字节替换为汇编指令
			//call 0x1b; retf;的二进制
			int eip=0;
			if (info->size >= 8 && strncmp(code + 4, "Hari", 4) == 0) {
				eip=0x1b;//应用程序的main入口
			}
			//&(task->tss.esp0)
			//需要在当前任务的TSS段中将esp0和ss写入操作系统当前的esp和ss
			start_app(eip, gdt_code, 64*1024, gdt_data, &(task->tss.esp0));
