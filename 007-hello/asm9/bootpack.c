...
void task_console(sSHEET *sht_back)
{
...
{	
	...
	sheet_init(sht_win, winbuf, 144, 52, -1);	//99感觉是随意设置的		
	
	init_mouse_cursor8(mouse_buf, 99);//99为背景色号
	init_screen(backbuf,binfo->scrnx,binfo->scrny);
	make_window8(winbuf, 144, 52, "task_a_input");	
	...
	for(i=0;i<3;i++){
		...
		winbuf = (uchar *)mm_alloc(memManager, 144 * 52);
		sheet_init(sht_win_b[i], winbuf, 144, 52, -1);
		sprintf(str,"task_b_%d",i+1);
		make_window8(winbuf, 144, 52, str);
		task_b[i] = task_alloc(memManager);
		task_b_esp = mm_alloc(memManager, 64 * 1024) + 64 * 1024;
		*(int *)(task_b_esp-8)=(int *)sht_win_b[i];
		task_b[i]->tss.esp = task_b_esp -12;
		task_b[i]->tss.es = 1 * 8;	
		task_b[i]->tss.cs = 2 * 8;
		task_b[i]->tss.ss = 1 * 8;
		task_b[i]->tss.ds = 1 * 8;
		task_b[i]->tss.fs = 1 * 8;
		task_b[i]->tss.gs = 1 * 8;
		task_b[i]->tss.eip = (int) &task_console;
		task_run(task_b[i], 1, i+2);
	}

	//task_run(task_b[0], 1, 3);

	sheet_slide(sht_win,  80, 160);
	sheet_slide(sht_win_b[0],240,160);
	sheet_slide(sht_win_b[1],80,220);
	sheet_slide(sht_win_b[2],240,220);
