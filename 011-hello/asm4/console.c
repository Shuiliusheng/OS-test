...
int hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		show_string_sheet(cons, (char*)(ecx+ds_base), 1);
	}
	else if(edx ==4){	//ret
		...
		if(reg[7]!=0){
			...
			//记录下来，用于退出时的释放
			sHandlerInfo *handler = mm_alloc(man_cons, sizeof(sHandlerInfo));
			handler->addr = (int)reg[7];
			handler->man = man;
			handler->type = HANDLER_MALLOC;
		...
		if(reg[7]!=0){
			...
			sHandlerInfo *handler = mm_alloc(man_cons, sizeof(sHandlerInfo));
			handler->addr = (int)reg[7];
			handler->man = man_cons;
			handler->type = HANDLER_FILE;
			handler->next = task->handler;
			task->handler = handler;
		}
	}
		...
		if(fifo32_status(&task->fifo)==0){
			...
			if(key==1||key==0){
				...
				if(key == BACKSPACE){	
					...
					cmdline[(cons.cursorX-cons.inputx)/8]='\0';
					...
					if(cons.cursorX<=(cons.maxW-8)){
						str[0]=key;
						str[1]=0;
						if( (cons.cursorX-cons.inputx)/8 < 127){
							cmdline[(cons.cursorX-cons.inputx)/8]=key;
						}
...
{
	boxfill8(cons->sheet->buf, cons->sheet->bxsize, COL8_000000, cons->sx, cons->sy, cons->maxW+8, cons->maxH);
	sheet_refresh(cons->sheet, cons->sx, cons->sy, cons->maxW+8, cons->maxH);
...
void cmd_type(sCONSOLEINFO *cons, char *cmdline, uint *fat)
{
	...
	if(info!=0){
		...
		char str[20];
		sFileInfoS *info = index_fileinfo(task->currentDir);
		sprintf(str,"currentDir: %d,%s",task->currentDir, info->filename);
		show_string(str,3);
