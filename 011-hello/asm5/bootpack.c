...
sSHEET *open_console(sSHTCTL *shtctl, uint memtotal, int w, int h)
{
	...
	winbuf = (uchar *)mm_alloc(memManager, w * h);
	...
	*(int *)(task_b_esp-12)=(int)sht_console;
	...
	fifo32_init(&task_console->fifo, 128, fifobuf, task_console);
...
{	
	...
	////////// set task
	sSHEET *key_win;
	key_win = open_console(shtctl, memtotal, 400,500);
	keywin_on(key_win);
	...
	for(;;){
		...
		if(fifo32_status(&fifo)==0){ //空的
			...
			if(key>=256&&key<=511){
				...
				if (key_ctrl ==1 && str[0] == 'c' && key_win->task->tss.ss0 != 0) {	
