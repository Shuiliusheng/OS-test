...
{	
	...
	winbuf = (uchar *)mm_alloc(memManager, 256 * 465);
	sheet_init(sht_console, winbuf, 256, 465, -1);
	make_window8(winbuf, 256, 465, "console",0);
	...
	*(int *)(task_b_esp-8)=(int)task_console;
	*(int *)(task_b_esp-12)=(int)sht_console;
	task_console->tss.esp = task_b_esp - 16;
	...
	sheet_slide(sht_win,  600, 160);
	sheet_slide(sht_console,  300, 100);
	...
	for(;;){
		...
		if(fifo32_status(&fifo)==0){ //空的
			...
			if(key>=256&&key<=511){
				key-=256;
				sprintf(str,"shift:%d, %d",key_shift, key_ctrl);
				show_string(str,29);
				...
				if (key == 0x2a) {	/* 左shift ON */
					...
					if(task_running==1){
						show_string("enter",21);
						fifo32_push(&(task_console->fifo),256+10);
					}
				}
					...
					if(task_running==0){
						task_running=1;
						cursor_color=-1;
						fifo32_push(&(task_console->fifo),4);
						...
						fifo32_push(&(task_console->fifo),3);
				...
				if (key_ctrl ==1 && str[0] == 'c' && task_console->tss.ss0 != 0) {	
					sCONSOLEINFO * cons = (sCONSOLEINFO *) *((int *) 0x0fec);
