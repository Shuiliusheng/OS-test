...
int addFreeBlock(struct MEM_MAN *man, sMEM_BLOCK *block);
int tryMergeFreeBlock(sMEM_BLOCK *block);
...
int addFreeBlock(struct MEM_MAN *man, sMEM_BLOCK *block)
{
	...
	while(freeblock!=NULL)
	{
		if((uint)block < (uint)freeblock){
			...
			merge_size=tryMergeFreeBlock(block);
			break;
		}
		else{
			//freeblock->next=NULL;
...
uchar mm_free(struct MEM_MAN *man, uint addr)
{
	...
	int freesize = block->size;
	block->size = size;
	block->flags=BLOCK_FREE;
	freesize+=addFreeBlock(man, block);	//合并增加的大小
	return freesize;
}
