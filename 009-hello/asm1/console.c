				...
				if(key == BACKSPACE){	
					...
					if(strcmp(cmdline,"mem")==0){
						...
						if(info!=0){
							...
							show_string_sheet(sheet,"file is not exist!",&cursor_x,&cursor_y,start_x,start_y, maxwidth, maxheight,1);
						}
					}
					else if(strcmp(cmdline,"hlt")==0){
						cursor_x=8;

						sFILEINFO *info=find_file(fileinfo,"HLT.HRB");
						if(info!=0){
							...
							show_string_sheet(sheet,"file is not exist!",&cursor_x,&cursor_y,start_x,start_y, maxwidth, maxheight,0);
						}
						cursor_y=console_newline(sheet,cursor_y, start_x, start_y,maxwidth, maxheight);
					}
