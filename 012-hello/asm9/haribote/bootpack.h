...
uint mm_freesize(struct MEM_MAN *man);
...
#define HANDLER_FILE 	2
#define HANDLER_CONS	3
#define HANDLER_TIMER	4
#define HANDLER_TIMER_APP	5
#define HANDLER_SHEET_APP	6
struct HANDLERINFO{
	int addr;
	int type;
	struct MEM_MAN *man;
...
#define sHandlerInfo struct HANDLERINFO
...
struct TASK{
	...
	int gdt_code;
	int gdt_data;
	sHandlerInfo *handler;
	int currentDir;
	char dir[6];

	//记录console当前是否正在进行 内存分配，文件分配等资源分配行为
	uchar flag_allocRes;
...
sTASK *task_now();
sTASK *find_task(int selector, int level);
...
//disk , 没有实现保护模式下的硬盘读写，只是读写了内存
#define SECTOR_INVALID 	0xffffffff
#define SECTOR_FREE 	0xfffffffe
struct DISKIINFO{
	uint addr;
	uint sec_num;
	uint free_sec;
...
/////////// filesystem
#define FILEHANDLER_R	1
#define FILEHANDLER_W	2
#define FILEHANDLER_A	3
#define FILEHANDLER_FREE 4
...
#define FILEINFO_FREE 0xff
#define FILEINFO_WBUSY 0xa0
#define FILEINFO_RBUSY 0xb0
#define FILEINFO_FILE 0x01
#define FILEINFO_DIR  0x02
#define FILEINFO_ALLOC  0x03
struct FILEINFO_SELF{
	char filename[19];
	uchar type;		/* 0x1:file, 0x2:dir, 0xff: free, 0xex: used*/
	uint sec;
	uint size;	//如果是目录，则用于记录busy的个数
	uint ndir;
};
#define sFileHandler struct FILE_HANDLER
#define sFileInfoS	struct FILEINFO_SELF
