	...
	while(1){
		...
		if(fifo32_status(stask->mousefifo)==0){
			...
			if(key >= 1024 && key <= 1030){
				...
				if(key == 1024){//left down
					event->type = MOUSE_EVENT_LEFTDOWN;
				}
				else if(key == 1025){
					event->type = MOUSE_EVENT_LEFTUP;
				}
				else if(key == 1026){
					event->type = MOUSE_EVENT_RIGHTDOWN;
				}
				else if(key == 1027){
					event->type = MOUSE_EVENT_RIGHTUP;
				}
				else if(key == 1028){
