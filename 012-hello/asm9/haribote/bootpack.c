...
sSHEET *open_console(sSHTCTL *shtctl, sFIFO32 *fifo, uint memtotal, int w, int h)
{
	...
	make_textbox8(sht_console, 8, 28, w-16, h-38, COL8_000000);
...
{	
	...
	sTIMER *timer=timer_alloc(memManager);
	timer_init(timer, &fifo, 1025);
	timer_settime(timer, 15);
	int sendx=0,sendy=0;
	...
	uint sheet_index=0,task_addr=0;
	for(;;){
		...
		if(fifo32_status(&fifo)==0){ //空的
			...
			if(key>=256&&key<=511){
				...
				if(mouse_decode(&mouseDec,key)==1){
					...
					key_win->task->mouse_event.x = mouseX - key_win->vx0;
					key_win->task->mouse_event.y = mouseY - key_win->vy0;
					
					if ((mouseDec.btn & 0x01) != 0) {
						if(key_win->task->mousefifo!=0 && leftdown==0){
							leftdown=1;
							key_win->task->mouse_event.type = MOUSE_EVENT_LEFTDOWN;
							fifo32_push((key_win->task->mousefifo),1024);
						}
						...
						if(key_win->task->mousefifo!=0&&leftdown==1){
							leftdown=0;
							key_win->task->mouse_event.type = MOUSE_EVENT_LEFTUP;
							fifo32_push((key_win->task->mousefifo),1025);
						}
						lastX = -1;
						lastY = -1;
					}
					...
					if ((mouseDec.btn & 0x02) != 0) {
						if(key_win->task->mousefifo!=0&&rightdown==0){
							key_win->task->mouse_event.type = MOUSE_EVENT_RIGHTDOWN;
							rightdown = 1;
							fifo32_push((key_win->task->mousefifo),1026);
						}
					}
					else{
						if(key_win->task->mousefifo!=0&&rightdown ==1){
							key_win->task->mouse_event.type = MOUSE_EVENT_RIGHTUP;
							rightdown = 0;
							fifo32_push((key_win->task->mousefifo),1027);
						}
					}
			...
			else if (key >= 10000){//task to harimain
				key -=10000;
				if(task_addr==0){
					task_addr = (uint)key;
				}
				else{
					task_addr = task_addr + ((uint)key<<16);
					sTASK *temp = (sTASK *)task_addr;
					task_addr=0;
					while(temp->flag_allocRes==1);
					close_task(temp);
				}
			}
			else if(key == 1025){
				
				if(lastX!=-1&&(lastX!=mouseX||lastY!=mouseY)){
				...
				if(key_win->task->mousefifo!=0 && (sendx!=mouseX||sendy!=mouseY)){
					key_win->task->mouse_event.type = MOUSE_EVENT_MOVE;
					fifo32_push((key_win->task->mousefifo),1028);
					sendx = mouseX;
					sendy = mouseY;
				}
