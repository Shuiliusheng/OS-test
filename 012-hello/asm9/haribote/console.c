...
char wait_key_input(int eax);
int wait_str_input(char *input, int ecx);
...
int hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		reg[7]=wait_str_input(input, ecx);
	}
		...
		//ecx: stack_addr
		//ebx: parameter
		reg[7] = (int)thread_create(task, ecx, eax, (int) &thread_app);
	}
	...
	else if(edx == 30){//create window
		// eax: w
		// ecx: h
		// ebx: buf
		// esi: title
		// edi: 透明色
		ds_base = get_segBaseAddr(task->gdt_data/8);
		
		task->flag_allocRes=1;
		sSHEET *sht = sheet_alloc(task->shtctl);		
		allocHandler(task, 0, sht, HANDLER_SHEET_APP);
		task->flag_allocRes=0;
		
		sheet_init(sht, (char*)(ebx+ds_base), eax, ecx, edi);
		make_window8((char*)(ebx+ds_base), eax, ecx, (char*)(esi+ds_base),0);
		if(task->cons->sheet==0){
			sheet_slide(sht, task->shtctl->vxsize/2,  task->shtctl->vysize/2);
		}
		else{
			sheet_slide(sht, task->cons->sheet->vx0,  task->cons->sheet->vy0);
		}
		sht->task = task;
		keywin_on(sht);
		...
		// edi:str
		ds_base = get_segBaseAddr(task->gdt_data/8);
		show_string_xy((sSHEET *)ebx,(char*)(ds_base+edi),eax,ecx,esi,ebp);
	}
	else if(edx == 32){//绘制方块区
		// ebx:sht
		// eax:x
		// ecx:y
		// esi:w
		// edi:h
		// ebp:color
		sSHEET *sht = (sSHEET *)ebx;
		boxfill8(sht->buf, sht->bxsize, ebp, eax, ecx, eax+esi, ecx+edi);
		sheet_refresh(sht, eax, ecx, eax+esi+1, ecx+edi+1);
	}
	else if(edx == 33){//刷新区域
		//ebx: sht
		//eax: x
		...
		//ecx: stack_addr
		//ebx: parameter
		reg[7] = (int)thread_create(task, ecx, eax, (int) &thread_event);
		thread_run((sThread *)reg[7]);
	}
	return 0;
}
...
{	
	...
	for(;;){
		...
		if(fifo32_status(task->fifo)==0){
			...
			else if(cons.sheet!=0&&key==3){//停止闪烁
				cons.color=-1;
	...
	if(info!=0){
		...
		if(task->gdt_code!= 0 && task->gdt_data!=0){
			...
			if (info->size >= 36 && strncmp(code + 4, "Hari", 4) == 0 && code!=0){
				...
				releaseHandler(task, HANDLER_SHEET_APP);
				task->flag_allocRes = 0 ;
...
void cmd_exit()
{
	...
	if (cons->sheet != 0) {
		...
		fifo32_push(task->os_fifo, ((uint)task&0xffff)+10000 );	// >=10000
		fifo32_push(task->os_fifo, (((uint)task>>16)&0xffff)+10000 );	// >=10000
	}
	io_sti();
...
void cmd_ncstart(sCONSOLEINFO *cons, char *cmdline, uint memtotal)
{
	sTASK *ftask = task_now();
	sTASK *task = open_task(ftask->shtctl,0, ftask->os_fifo, memtotal);
	
	int i=0;
	for(i=0;i<strlen(cmdline);i++){
		fifo32_push((task->fifo),cmdline[i]+256);
	}
	fifo32_push((task->fifo),10+256);//enter
	
	cons->cursorX=8;
	console_newline(cons);
}

void cmd_cons(sCONSOLEINFO *cons, char *cmdline, uint memtotal)
{
	int index = 0;
	int w=str2int(cmdline,&index);
	int h=str2int(cmdline,&index);
	if(w<=80){
		w=400;
	}
	if(h<=80){
	...
	sTASK *task = task_now();
	sSHEET *sht = open_console(task->shtctl, task->os_fifo, memtotal, w, h);
...
{
	if(cons->sheet!=0&&strcmp(cmdline,"mem")==0){
		cmd_mem(cons, memtotal);
	}
	else if(cons->sheet!=0&&strcmp(cmdline,"cls")==0){
		cmd_cls(cons);
	}
	else if(cons->sheet!=0&&strcmp(cmdline,"dir")==0){
		cmd_dir(cons);
	}
	else if(cons->sheet!=0&&strcmp_sub(cmdline,"type ")==0){
		cmd_type(cons,cmdline,fat);
	}
	else if(strcmp_sub(cmdline,"start ")==0){
		cmd_start(cons,&cmdline[6],memtotal);
	}
	else if(strcmp_sub(cmdline,"ncstart ")==0){
		cmd_ncstart(cons,&cmdline[8],memtotal);
	}
	else if(strcmp_sub(cmdline,"cons ")==0){
		cmd_cons(cons,&cmdline[5],memtotal);
	}
	...
	else if(cons->sheet!=0&&strcmp(cmdline,"disk")==0){
		char str[50];
		sprintf(str,"sector used: %d", used_sector_num());
		show_string_sheet(cons,str,1);
	}
	else if(cons->sheet!=0&&strcmp(cmdline,"fs")==0){
		char str[50];
		sprintf(str,"fileinfo used: %d", used_fileinfo());
		show_string_sheet(cons,str,1);
	}
	else if(cons->sheet!=0&&strcmp(cmdline,"gdt")==0){
		char str[50];
...
void console_newline(sCONSOLEINFO *cons)
{
	...
	if(cons->cursorY<=(cons->maxH-16)){
		...
		//上移
		if(cons->isType==1){
			key = wait_key_input(1);
		}
		if(key == '2'){
			t1 = (cons->cursorY - cons->sy)/16;
			y = cons->sy + (t1/2)*16;
			key = cons->cursorY + 16 - y;
			cons->cursorY = y;
		}
		else if(key == '3'){
			key = cons->cursorY + 16 - cons->sy;
			cons->cursorY = cons->sy;
		}
		else if(key == '4'){
			key = cons->cursorY + 16 - cons->sy;
		...
		t = cons->sy*cons->sheet->bxsize;
		t1=(cons->sy+key)*cons->sheet->bxsize;
		for(y=cons->sy;y<=cons->cursorY;y++){
			for(x=cons->sx;x<=t2;x++){
				cons->sheet->buf[t+x]=cons->sheet->buf[t1+x];
			}
			t+=cons->sheet->bxsize;
			t1+=cons->sheet->bxsize;
		}
		
		t=cons->cursorY*cons->sheet->bxsize;
		for(y=cons->cursorY;y<=cons->cursorY+key;y++){
			for(x=cons->sx;x<=t2;x++){
				cons->sheet->buf[t+x]=COL8_000000;
			}
			t+=cons->sheet->bxsize;
		}
		sheet_refresh(cons->sheet,cons->sx, cons->sy,cons->sheet->bxsize-8,cons->cursorY+key);
	}
}
...
char wait_key_input(int eax)
{
	...
	while(1){
		...
		if(fifo32_status(task->fifo)==0){
			...
			key=fifo32_pop(task->fifo);
			io_sti();
			if(task->cons->sheet!=0&&key<=1){
				timer_init(task->cons->timer, task->fifo, 1);
				timer_settime(task->cons->timer, 50);
			}
			else if(task->cons->sheet!=0&&key==3){//停止闪烁
				task->cons->color=-1;
			}
			else if(task->cons->sheet!=0&&key==4){//开始闪烁
				task->cons->color=COL8_000000;
			}
			else if(key==5){//exit
				cmd_exit();
			}
		...
		if(fifo32_status(task->fifo)==0){
			...
			if(task->cons->sheet!=0&&key<=1){
				...
				if(key == BACKSPACE){	
					if(index>0){
						index--;
						if(task->cons->cursorX==8){
							task->cons->cursorX=task->cons->maxW;
							task->cons->cursorY-=16;
						}
						else{
							task->cons->cursorX-=8;
						}
						if(task->cons->sheet!=0){
							show_string_sheet(task->cons," ",0);
						}
					...
					if(index<ecx-1){
						...
						str[0]=key;
						if(task->cons->sheet!=0){
							show_string_sheet(task->cons,str,0);
						}
					}
					else{
						input[index]=key;
