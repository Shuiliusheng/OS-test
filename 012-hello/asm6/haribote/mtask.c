...
void releaseHandlerSingle(sTASK *task, sHandlerInfo *handler)
{
	...
	if(handler->type == HANDLER_TIMER||handler->type == HANDLER_TIMER_APP){
		timer_free(handler->man, (sTIMER *)handler->addr);
	}
	else if(handler->type == HANDLER_SHEET_APP){
		sheet_free((sSHEET *)handler->addr);
	}
	else{
...
void releaseHandler(sTASK *task, int type)
{
	...
	while(info!=0){
		if(info->type == type){
			if(type == HANDLER_TIMER||type == HANDLER_TIMER_APP){
			...
			else if(info->type == HANDLER_SHEET_APP){
				sheet_free((sSHEET *)info->addr);
			}
			else{
				mm_free(info->man, (char*)info->addr);
				mm++;
...
int handlerNum()
{
	...
	int mm=0,hf=0,hc=0,ht=0;
	while(info!=0){
		if(info->type == HANDLER_TIMER||info->type == HANDLER_TIMER_APP){
			ht++;
		}
