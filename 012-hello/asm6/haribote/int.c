...
{
	sCONSOLEINFO *cons=(sCONSOLEINFO *)(*((int *)(ADR_CONS)));
	show_string_sheet(cons,"General protection exception", 1);
	char str[20];
	sprintf(str,"EIP = %08x",esp[11]);
...
{
	sCONSOLEINFO *cons=(sCONSOLEINFO *)(*((int *)(ADR_CONS)));
	show_string_sheet(cons,"Stack exception", 1);
	char str[20];
	sprintf(str,"EIP = %08x",esp[11]);
	show_string_sheet(cons,str, 1);
	sTASK *task=task_now();
